...
class TicTacToe:
    def __init__(self):
        self.board = [' ' for _ in range(9)]  # Representing the 3x3 grid
...
class SmartComputerPlayer:
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            square = random.choice(game.available_moves())  # Choose random move if it's the first move
        else:
    ...
    def minimax(self, state, player):
        ...
        for possible_move in state.available_moves():
            # Make a move and try it recursively
